            ...
            <div className="col-9" style={{ width: "100%" }}>
                <div className="btn-group mt-2 mb-5">
                    ...
                    <select className="form-select" value={selectedStatus} onChange={(e) => { setSelectedStatus(e.target.value) }}>
                                ...
                                {
                                    [...new Set(userdata.map(item => (item.applicationstatus.status)))].map(item => (
                    ...
                    <select className="form-select" value={selectedAdmitCard} onChange={(e) => { setSelectedAdmitCard(e.target.value) }}>
                                ...
                                {
                                    [...new Set(userdata.map(item => (item.admitcard.status)))].map(item => (
                                        <option value={item}>{item}</option>
                                    ))
                                }
                            </select>
                        ...
                        <tbody>
                            {filteredData.map((item, index) => (
                                <tr key={index} onClick={()=>{btnappnoclicked(item.applicationId)}} style={{cursor:"pointer"}}>
                                    ...
                                    <td>{item.admitcard.OfficerName}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
